...
public class UserServiceImpl implements UserService {
	...
	final static String[] FORBID_NAME = new String[]{"xinbei,xinbei51,51xinbei,admin"};
...
//		Assert.hasLength(user.getEmail(), "邮箱不能为空!");
		...
		for(String fn : FORBID_NAME){
			if(getUsername.contains(fn)){
				throw new RuntimeException("用户名不能包含本站关键词！");
			}
		}
		
		String getName = user.getName();
		for(String fn : FORBID_NAME){
			if(getName.contains(fn)){
				throw new RuntimeException("昵称不能包含本站关键词！");
			}
		}
